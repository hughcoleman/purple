...
class TestSwitch(unittest.TestCase):
    def test__illegal_routing_logic(self):
        ...
        illegal_routing_logic = {0: [0, 1, 2], 1: [2, 1], 2: [1, 1, 2]}

        self.assertRaises(
            ValueError,
            system97.switch.SteppingSwitch,
            {"routing_logic": illegal_routing_logic, "position": 0, "size": 3},
        )
    ...
    def test__illegal_arm_position(self):
        ...
        illegal_positions = [-1, 25, 99]

        for position in illegal_positions:
            self.assertRaises(
                ValueError,
                system97.switch.SteppingSwitch,
                {"routing_logic": {}, "position": position},
            )
